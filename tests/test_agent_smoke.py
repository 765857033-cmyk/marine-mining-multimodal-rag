...
class AgentSmokeTests(unittest.TestCase):
    def test_agent_returns_sources_for_domain_question(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any("混合检索" in step for step in answer.trace))
            self.assertTrue(any("证据验证" in step for step in answer.trace))
            self.assertTrue(any(event["node"] == "引用校验" for event in answer.trace_events))
